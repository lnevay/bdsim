  ...
  while (n < maxTries) // prevent infinite loops
    {
      ...
      if ((coords.local.totalEnergy - particleDefinition->Mass()) > 0 or ((BDSGlobalConstants::Instance()->RadioactiveDecay() and (coords.local.totalEnergy - particleDefinition->Mass()) == 0)))
	{break;}
    }
  if (n >= maxTries)
