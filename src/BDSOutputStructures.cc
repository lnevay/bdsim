...
G4int BDSOutputStructures::Create1DHistogramRun(const G4String& name, const G4String& title,
						G4int nbins, G4double xmin, G4double xmax)
{
  return runHistos->Create1DHistogram(name, title, nbins, xmin, xmax);
}

G4int BDSOutputStructures::Create1DHistogramEvent(const G4String& name, const G4String& title,
						  std::vector<double>& edges)
{
  return evtHistos->Create1DHistogram(name,title,edges);}
}

G4int BDSOutputStructures::Create1DHistogramRun(const G4String& name, const G4String& title,
						std::vector<double>& edges)
{
  return runHistos->Create1DHistogram(name,title,edges);}
}

G4int BDSOutputStructures::Create3DHistogramEvent(const G4String& name, const G4String& title,
						  G4int nBinsX, G4double xMin, G4double xMax,
						  G4int nBinsY, G4double yMin, G4double yMax,
						  G4int nBinsZ, G4double zMin, G4double zMax)
{
  return evtHistos->Create3DHistogram(name, title,
				      nBinsX, xMin, xMax,
				      nBinsY, yMin, yMax,
				      nBinsZ, zMin, zMax);
}

G4int BDSOutputStructures::Create3DHistogramRun(const G4String& name, const G4String& title,
						G4int nBinsX, G4double xMin, G4double xMax,
						G4int nBinsY, G4double yMin, G4double yMax,
						G4int nBinsZ, G4double zMin, G4double zMax)
{
  return runHistos->Create3DHistogram(name, title,
				      nBinsX, xMin, xMax,
				      nBinsY, yMin, yMax,
				      nBinsZ, zMin, zMax);
}
